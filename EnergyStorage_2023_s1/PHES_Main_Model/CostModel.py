...
class PHESCostModel:
    ...
    def __get_capital_cost(self):
        usd = self.__result["reservoir_cost"][0] + self.__result["tunnel_cost"][0] + self.__result["powerhouse_cost"][0]
        aud = usd / self.exchange_rate_to_USD
        self.__result["capital_cost"] = (usd, aud)
        return self.__result["capital_cost"]

    def __get_reservoirs_to_capital(self):
        self.__result["reservoirs_to_capital"] = self.__result["reservoir_cost"][0] / self.__result["capital_cost"][0] * 100
        return self.__result["reservoirs_to_capital"]

    def __get_tunnel_to_capital(self):
        self.__result["tunnel_to_capital"] = self.__result["tunnel_cost"][0] / self.__result["capital_cost"][0] * 100
        return self.__result["tunnel_to_capital"]

    def __get_powerhouse_to_capital(self):
        self.__result["powerhouse_to_capital"] = self.__result["powerhouse_cost"][0] / self.__result["capital_cost"][0] * 100
    ...
    def __get_capital_cost_mwh(self):
        usd = ((self.__result["capital_cost"][0] + self.__result["npv_refurbishment"][0])
               * self.__result["real_discount_rate"]) / (1 - np.exp(-1 * self.__result["real_discount_rate"] *
                                                                self.system_life)) / self.__result["annual_energy_sold"]
        aud = usd / self.exchange_rate_to_USD
    ...
    def __get_om_cost_mwh(self):
        usd = self.__result["annual_om_rate"] * self.__result["capital_cost"][0] / self.__result["annual_energy_sold"]
        aud = usd / self.exchange_rate_to_USD
    ...
    def __get_inital_outlay(self):
        self.__initial_outlay = -1 * (self.__result["capital_cost"][0] + self.__result["npv_refurbishment"][0]) * 1000000
